...
def evaluate(args, model, eval_dataset, mode, global_step=None):
    ...
    for batch in progress_bar(eval_dataloader):
        ...
        with torch.no_grad():
            ...
            # tags = model.crf.decode(logits, inputs["attention_mask"], pad_tag=-100)
            # tags = model.crf.decode(logits, inputs["attention_mask"])
            tags = model.crf.decode(logits, inputs["crf_mask"])
            # print(f"Before tags : {tags}")
            # print(f"Before tags.shape : {tags.shape}")
            tags = tags.squeeze(0).cpu().numpy().tolist()
            # print(f"After squeeze tags : {tags}")
            # print(f"After squeeze len(tags), len(tags[0]) : {len(tags)}, {len(tags[0])}")
